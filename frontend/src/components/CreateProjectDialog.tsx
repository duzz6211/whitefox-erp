...
import { createProject, fetchCompanies } from '../api/endpoints';
import type { Company, ProjectCategory } from '../types';
...
export default function CreateProjectDialog({ onClose, onCreated }: Props) {
  ...
  const [category, setCategory] = useState<ProjectCategory>('internal_ops');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err.response?.data?.detail ?? '프로젝트 생성 실패');
    } finally {
      setBusy(false);
    }
  }
    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={onClose}>
      ...
      >
        ...
        <label className="block text-sm font-medium mb-1">
          연결 고객사 <span className="text-slate-400 text-xs">(선택)</span>
        </label>
        <select
          className="w-full border border-slate-300 rounded px-3 py-2 mb-4"
          value={companyId}
